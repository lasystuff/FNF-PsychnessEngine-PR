...
class ModsMenuState extends MusicBeatState
{
	...
	var conflictWarningText:FlxText;
	...
	override function create()
	{
		...
		conflictWarningText = new FlxText(bgDescription.x + 15, bgDescription.y + bgDescription.height - myHeight - 65, bgDescription.width - 30, Language.getPhrase('mod_competition', '* If you play with this mod, it is recommended to disable other mods, as it may conflict with other mods!'), 16);
		conflictWarningText.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.YELLOW, RIGHT);
		add(conflictWarningText);
	...
	override function update(elapsed:Float)
	{
		...
		try
		{
			var curMod:ModItem = modsGroup.members[curSelectedMod];
			if(curMod != null)
				bgHeader.alpha = FlxMath.lerp(FileSystem.exists(Paths.mods('${curMod.folder}/header.png')) ? 1 : 0, bgHeader.alpha, Math.exp(-elapsed * 5));
			blackScreen.alpha = FlxMath.lerp(0, blackScreen.alpha, Math.exp(-elapsed * 1));
		}
		catch(e) {}
	...
	function updateModDisplayData(fadeBlackScreen:Bool = true)
	{
		var curMod:ModItem = modsGroup.members[curSelectedMod];
		...
		var isPixel = false;
		var file:String = Paths.mods('${curMod.folder}/header.png');
		if (!FileSystem.exists(file))
		{
			file = Paths.mods('${curMod.folder}/header-pixel.png');
			isPixel = true;
		}
		...
		if(FileSystem.exists(file))
		{
			if (fadeBlackScreen)
				blackScreen.alpha = 1;
			bgHeader.loadGraphic(Paths.cacheBitmap(file, bmp));
		...
		conflictWarningText.visible = curMod.competitionWarning;
...
class ModItem extends FlxSpriteGroup
{
	...
	public var competitionWarning:Bool = false;
	...
	public function new(folder:String)
	{
		...
		if(pack != null)
		{
			...
			this.competitionWarning = (pack.competitionWarning == true);
		}
		text.text = this.name;
