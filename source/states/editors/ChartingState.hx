...
enum abstract UndoAction(String)
{
	...
	var ADD_CHARACTER = 'Add Character';
	var DELETE_CHARACTER = 'Delete Character';
	var MOVE_CHARACTER = 'Move Character';
}
...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	public static final defaultEvents:Array<Array<String>> = [
		...
		[
			'Change Camera Modulo',
			"Sets the camera to zoom every specified number of beats.\nValue 1: Number of beats between each zoom."
		],
		[
			'Add Camera Zoom',
	...
	public static var keysArray:Array<FlxKey> = [ONE, TWO, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT]; // Used for Vortex
	public static var SHOW_EVENT_COLUMN = true;
	public static var GRID_COLUMNS_PER_PLAYER = 4;
	public static var GRID_PLAYERS(default, set) = 3;
	public static var GRID_SIZE = 32;

	static function set_GRID_PLAYERS(v:Int)
	{
		@:privateAccess
		{
			GRID_PLAYERS = v;
			...
			instance.reloadStrumNotes();
			instance.reloadIcons();

			for (note in instance.notes)
			{
				if (note == null)
					continue;
				instance.positionNoteXByData(note);
			}

			for (note in instance.events)
			{
				if (note == null)
					continue;
				note.x = instance.gridBg.x;
				note.eventText.x = note.x - note.eventText.width - 10;
			}

			new FlxTimer().start(0.01, function(tmr:FlxTimer)
			{
				instance.loadSection();
				instance.updateGridVisibility();
			});
		}
		return GRID_PLAYERS;
	}
	...
	var characterListBox:PsychUIBox;
	var characterListBoxPosition:FlxPoint = FlxPoint.get(40, 360);
	var upperBox:PsychUIBox;

	var characterList:Array<String> = [];
	var charListRadioGroup:PsychUIRadioGroup;
	...
	override function create()
	{
		...
		infoBox = new PsychUIBox(infoBoxPosition.x, infoBoxPosition.y, 220, 223, ['Information']);
		...
		characterListBox = new PsychUIBox(characterListBoxPosition.x, characterListBoxPosition.y, 250, 230, ['Character List']);
		characterListBox.scrollFactor.set();
		characterListBox.cameras = [camUI];
		add(characterListBox);
		...
		if (chartEditorSave.data.characterListBoxPosition != null && chartEditorSave.data.characterListBoxPosition.length > 1)
			characterListBox.setPosition(chartEditorSave.data.characterListBoxPosition[0], chartEditorSave.data.characterListBoxPosition[1]);
		...
		{
			GRID_PLAYERS = PlayState.SONG.characters.length;
			reloadCharacterList();
			charListRadioGroup.checked = 0;
		}
		...
		fullTipText.text = [
			...
			"Ctrl + O - Select all Opponent Notes in current Section",
			"Ctrl + P - Select all Player Notes in current Section",
			"Ctrl + S - Quicksave",
		].join('\n');
		...
		{
			displayUnsavedWarningPrompt(function() Sys.exit(0));
		};
		FlxG.watch.add(this, 'chartSaved');
		super.create();
	}

	function displayUnsavedWarningPrompt(onClose:Void->Void)
	{
		if (!chartSaved && !ignoreProgressCheckBox.checked)
		{
	...
	function reloadIcons()
	{
		...
		if (SHOW_EVENT_COLUMN)
		{
			...
			// text.x = eventIcon.x + eventIcon.width / 2 - text.width / 2;

			columns++;
		}

		var gridStripes:Array<Int> = [];
		var textStrings:Array<String> = ["Player", "Opponent"];
		for (i in 0...GRID_PLAYERS)
		{
			...
			// var text:FlxText = new FlxText();
			// text.y = 100;
			// text.text = textStrings[i];
			// text.scrollFactor.set();
			// add(text);

			// text.x = icon.x + icon.width / 2 - text.width / 2;
		}
		...
		if (PsychUIInputText.focusOn == null && lastFocus == null)
		{
			...
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				&& (FlxG.keys.justPressed.Z || FlxG.keys.justPressed.Y || FlxG.keys.justPressed.X || FlxG.keys.justPressed.C || FlxG.keys.justPressed.V
					|| FlxG.keys.justPressed.A || FlxG.keys.justPressed.S || FlxG.keys.justPressed.O || FlxG.keys.justPressed.P || FlxG.keys.justPressed.E))
			{
				canContinue = false;
				...
				else if (FlxG.keys.justPressed.A) // Select All (Ctrl + A)
				{
					...
					saveChart();
				else if (FlxG.keys.justPressed.O) // Select All Opponent Notes (Ctrl + O)
				{
					var sel = selectedNotes;
					...
					for (i in 0...curRenderedNotes.members.length)
					{
						...
						if (!note.mustPress && !note.gfNote && !(note is EventMetaNote))
							noteArray.push(note);
						else
				...
				else if (FlxG.keys.justPressed.P) // Select All Player Notes (Ctrl + P)
				{
					var sel = selectedNotes;
					var noteArray:Array<MetaNote> = [];
					for (i in 0...curRenderedNotes.members.length)
	...
	function loadChart(song:SwagSong, path:String)
	{
		if (focusCharDropDown != null)
			focusCharDropDown.selectedIndex = 0;
		PlayState.SONG = song;
		GRID_PLAYERS = PlayState.SONG.characters.length;
	...
	function reloadCharacterList()
	{
		...
		{
			var baseName = char.name;
			var finalName = baseName + '#${char.index}';
			...
			characterList.push(finalName + postfix);
		}
		charListRadioGroup.labels = characterList;
		if (focusCharDropDown != null)
			focusCharDropDown.list = characterList;
	}
	...
	function createNote(note:Dynamic, ?secNum:Null<Int> = null)
	{
		...
		for (i in 0...strumArray.length)
			gottaHitNote = strumArray[note[1]] == 1;
		var gottaGFNote:Bool = false;
		for (i in 0...strumArray.length)
			gottaGFNote = strumArray[note[1]] == 2;
		...
		swagNote.gfNote = gottaGFNote;
		swagNote.noteColumn = note[1];
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		if (sec != null)
		{
			focusCharDropDown.selectedIndex = sec.focusCharacter;
	...
	function updateJsonData():Void
	{
		for (i in 1...GRID_PLAYERS + 1)
		{
			...
			var data:CharacterFile = loadCharacterFile(Reflect.field(PlayState.SONG.characters[i - 1], 'name'));
	...
	function updateHeads(ignoreCheck:Bool = false):Void
	{
		...
		for (i in 0...GRID_PLAYERS)
		{
			...
			var iconName:String = Reflect.field(characterData, 'iconP${i + 1}');
	...
	function addChartingTab()
	{
		...
		playbackSlider.description = 'Playback Rate\n- Set Playback Rate';
		...
		mouseSnapCheckBox.description = 'Mouse Scroll Snap\n- If checked,\n   scrolling with the mouse wheel\n   will scroll along the grid';
		...
		ignoreProgressCheckBox.description = 'Ignore Progress Warnings\n- If checked,\n   all progress warnings\n   will be removed\n   (Recommend checking)';
		...
		opponentVolumeStepper.description = 'Opponent Vocals\n- Opponent Vocals volume';
	...
	function addDataTab()
	{
		...
		gameOverCharDropDown.description = 'Game Over Character\n- Character to display when the game over\n- Default: bf-dead';
		...
		gameOverSndInputText.description = 'Game Over Death Sound\n- Default: fnf_loss_sfx';
		objY += 40;
		...
		gameOverLoopInputText.description = 'Game Over Loop Music\n- Default: gameOver';
		objY += 40;
		...
		gameOverRetryInputText.description = 'Game Over Retry Music\n  - Default: gameOverEnd';

		objY += 35;
		noRGBCheckBox = new PsychUICheckBox(objX, objY, 'Disable Note RGB', 100, updateNotesRGB);
		noRGBCheckBox.description = 'Disable Note RGB\n  - Whether to isable RGB shaders for Note';
		...
		noteTextureInputText.description = 'Note Texture\n- Note Texture (images/)\n- Default: noteSkins/NOTE_assets\n  (If blank, it will be default)';
		...
		noteSplashesInputText.description = 'Note Splashes Texture\n- Note Splashes Texture (images/)\n- Default: noteSplashes/noteSplashes\n  (If blank, it will be default)';
	...
	function addEventsTab()
	{
		...
		eventDropDown.description = 'Event\n- Event to be triggered';
		...
		value1InputText.description = 'Value 1\n- Set Value 1 for the event\n- Warning: The value is a String!\n  - When used as an Int,\n     Haxe: Std.parseInt(value1)\n     Lua: tonumber(value1)\n  - When used as an Float,\n     Haxe: Std.parseFloat(value1)\n     Lua: tonumber(value1)';
		...
		value2InputText.description = 'Value 2\n- Set Value 2 for the event\n- Warning: The value is a String!\n  - When used as an Int,\n     Haxe: Std.parseInt(value2)\n     Lua: tonumber(value2)\n  - When used as an Float,\n     Haxe: Std.parseFloat(value2)\n     Lua: tonumber(value2)';
	...
	function addNoteTab()
	{
		...
		susLengthStepper.description = 'Sustain length\n- Sustain length of selected notes';
		...
		strumTimeStepper.description = 'Note Hit time (ms)\n- Note Hit time of selected notes';
		...
		noteTypeDropDown.description = 'Note Type\n- Note Type of selected notes';
	...
	var focusCharDropDown:PsychUIDropDownMenu;
	...
	function addSectionTab()
	{
		...
		mustHitCheckBox.description = 'Must Hit Section\n- Whether the camera will focus on Player';
		...
		changeBpmCheckBox.description = 'Change BPM\n- Whether to change bpm in this section';
		...
		pasteButton.description = 'Paste Section\n- Paste to current section';
		...
		clearButton.description = 'Clear Section\n- Clear current section';
		...
		affectNotes.description = 'Notes\n- Affect Notes?';
		affectEvents = new PsychUICheckBox(objX + 100, objY, 'Events', 60);
		affectEvents.description = 'Events\n- Affect Events?';
		...
		tab_group.add(new FlxText(focusCharDropDown.x, focusCharDropDown.y - 15, 100, 'Focus Character:'));
		tab_group.add(focusCharDropDown);
	}
	...
	var _maxIntervalLimit:Float = 2.0;
	...
	function addSongTab()
	{
		...
		songNameInputText.description = 'Song Name\n- Warning: Case is not important\n  - Symbols are not allowed in Song Audio and Song Data Folder,\n     but can be used in Song Name';
		...
		reloadJsonButton.description = 'Reload JSON\n- Load/Reload the JSON of the\n   song entered in the Song Name Box';
		// #end
		...
		scrollSpeedStepper.description = 'Scroll Speed\n- Song default Scroll Speed';
		...
		audioOffsetStepper.description = 'Audio Offset (ms)\n- Audio offset in milliseconds';

		objY += 25;
		var btn:PsychUIButton = new PsychUIButton(objX, objY, 'BPM Detection', function()
		{
			var currentTime:Float = FlxG.game.ticks / 1000;
			_tapTimes.push(currentTime);

			if (_tapTimes.length >= 2)
			{
				var lastInterval = currentTime - _tapTimes[_tapTimes.length - 2];
				if (lastInterval > _maxIntervalLimit)
				{
					_tapTimes = [currentTime];
					return;
				}
			}
			...
			if (_tapTimes.length >= 2)
			{
				...
				for (i in intervals)
					sum += i;
				var avgInterval:Float = sum / intervals.length;

				var bpm:Float = CoolUtil.floorDecimal(60.0 / avgInterval, 2);
				showOutput('Detected BPM: $bpm');
			}
		});
		btn.description = 'BPM Detection\n- Click two or more times to detect\n   the BPM based on the interval between clicks';
		tab_group.add(btn);
		...
		playerDropDown.description = 'Player\n- Default Player Character';
		stageDropDown = new PsychUIDropDownMenu(objX, objY, [''], function(id:Int, stage:String)
		{
			PlayState.SONG.stage = stage;
			StageData.loadDirectory(PlayState.SONG);
			trace('selected $stage');
		});
		stageDropDown.description = 'Stage';
		...
		opponentDropDown.description = 'Opponent\n- Default Opponent Character';
		...
		girlfriendDropDown.description = 'Girlfriend\n- Default Girlfriend Character';
	...
	function addFileTab()
	{
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Chart...', function()
		{
			...
			openSubState(new BasePrompt(620, 380, 'Open Chart Data', function(state:BasePrompt)
			{
				upperBox.isMinimized = true;
				upperBox.bg.visible = false;
				...
				var dropDownBG:FlxSprite = new FlxSprite(0, 0).makeGraphic(240, 240, 0x40FFFFFF);
				dropDownBG.cameras = state.cameras;
				dropDownBG.x = state.bg.x + 45;
				dropDownBG.y = state.bg.y + state.bg.height - dropDownBG.height - 45;
				state.add(dropDownBG);

				var dropDownText:FlxText = new FlxText(0, 0, 240, 'Drag and drop a chart\nfile (.json) here.', 16);
				dropDownText.alpha = 0.75;
				dropDownText.alignment = CENTER;
				dropDownText.cameras = state.cameras;
				dropDownText.x = dropDownBG.x + dropDownBG.width / 2 - dropDownText.width / 2;
				dropDownText.y = dropDownBG.y + dropDownBG.height / 2 - dropDownText.height / 2;
				state.add(dropDownText);

				function _loadChart(data:String, path:String)
				{
					try
					{
						var filePath:String = path.replace('\\', '/');
						var loadedChart:SwagSong = Song.parseJSON(data, filePath.substr(filePath.lastIndexOf('/')));
						if (loadedChart == null || !Reflect.hasField(loadedChart, 'song')) // Check if chart is ACTUALLY a chart and valid
						{
							showOutput('Error: File loaded is not a Psych Engine/FNF 0.2.x.x chart.', true);
							return;
						...
						var func:Void->Void = function()
						{
							loadChart(loadedChart, filePath);
				...
				var startText:FlxText = new FlxText(0, 0, 240, 'Browse', 16);
				startText.alignment = CENTER;
				startText.cameras = state.cameras;
				startText.x = state.bg.x + state.bg.width - startText.width - 45;
				startText.y = dropDownBG.y;
				state.add(startText);

				var btn:PsychUIButton = new PsychUIButton(0, 0, 'Browse Chart...', function()
				{
					fileDialog.open(function()
					{
						_loadChart(fileDialog.data, fileDialog.path);
					});
				}, 240);
				btn.cameras = state.cameras;
				btn.x = state.bg.x + state.bg.width - btn.width - 45;
				...
				nearestText.y = btn.y + btn.height + 10;
				state.add(nearestText);

				var nearestBG:FlxSprite = new FlxSprite(0, 0).makeGraphic(240, 148, 0x80000000);
				nearestBG.cameras = state.cameras;
				nearestBG.x = 665;
				nearestBG.y = 357;
				state.add(nearestBG);
				...
				for (i in 0...5)
				{
					if (realNearestList[i] != null)
					{
						...
						text.y = nearestText.y + nearestText.height + 15 + i * 20;
						text.alpha = 0.85;
						state.add(text);
				...
				var text:FlxText = new FlxText(0, 0, 240, 'More...', 8);
				text.cameras = state.cameras;
				text.color = 0xFF1F88D9;
				text.x = state.bg.x + state.bg.width - text.width - 32;
				text.y = nearestBG.y + nearestBG.height - text.height - 15;
				text.alpha = 0.85;
				state.add(text);
				...
				flixel.input.mouse.FlxMouseEvent.add(text, spr ->
				{
					...
					for (i in 0...radioNearestList.length)
					{
						...
						radioNearestList[i] = '{json}${jsonName}{json} - ${radioNearestList[i].substr(0, 42 - (jsonName.length + 3))}...';
					}
					...
					openSubState(new BasePrompt(420, hei, 'Choose an Nearest Chart', function(state:BasePrompt)
					{
						upperBox.isMinimized = true;
						upperBox.bg.visible = false;
				...
				Main.onDropFile = function(filePath:String)
				{
					_loadChart(File.getContent(filePath), filePath);
				}
			}, null, function(state:BasePrompt)
			{
				Main.onDropFile = null;
			}));
		}, btnWid);
		btn.description = 'Open Chart...\n- Open Chart in various ways';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Autosave...', function()
		{
			...
			openSubState(new BasePrompt(420, hei, 'Choose an Autosave', function(state:BasePrompt)
			{
				...
				var btn:PsychUIButton = new PsychUIButton(0, radioGrp.y + radioGrp.height + 20, 'Load', function()
				{
					var autosaveName:String = fileList[radioGrp.curScroll + radioGrp.checked];
					...
					if (FileSystem.exists(path))
					{
						try
						{
							...
							var func:Void->Void = function()
							{
								...
								loadChart(loadedChart, Song.chartPath);
								reloadNotesDropdowns();
								prepareReload();
		...
		btn.description = 'Open Autosave...\n- Select and load the chart in the buckups folder';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		if (SHOW_EVENT_COLUMN)
		{
			...
			btn.description = 'Open Events...\n- Open Event(events.json) and choose whether to replace all or add';
			btn.text.alignment = LEFT;
			tab_group.add(btn);
		}
		...
		FlxG.watch.add(btn, 'descriptionBox');
		...
		btn.description = 'Save as...\n- Save Chart to any location';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
			...
			var func:Void->Void = function()
			{
				...
				if (FileSystem.exists(Song.chartPath))
				{
					try
					{
						...
						loadChart(reloadedChart, Song.chartPath);
						reloadNotesDropdowns();
						prepareReload();
		...
		btn.description = 'Save (V-Slice)...\n- Save Chart and Metadata for V-Slice';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		btn.description = 'Psych to V-Slice...\n- Select Psych Engine chart and save it for V-Slice';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		btn.description = 'V-Slice to Psych...\n- Select Chart and Metadata for V-Slice and save for Psych Engine';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		btn.description = 'Update (Legacy)...\n- Select Psych Engine 0.x.x/FNF 0.2.x.x Chart and save Psych Engine 1.x.x';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		btn.description = 'Exit\n- Leaving Chart Editor';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
	}
	...
	function addEditTab()
	{
		...
		if (SHOW_EVENT_COLUMN)
		{
			...
			btn.description = 'Lock/Unlock Events';
			btn.text.alignment = LEFT;
			tab_group.add(btn);
		}
		...
		btn.description = 'Open Autosave Settings';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		btn.description = 'Clear All Notes';
		btn.normalStyle.bgColor = FlxColor.RED;
		btn.normalStyle.textColor = FlxColor.WHITE;
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		if (SHOW_EVENT_COLUMN)
		{
			...
			btn.description = 'Clear All Events';
			btn.normalStyle.bgColor = FlxColor.RED;
			btn.normalStyle.textColor = FlxColor.WHITE;
			btn.text.alignment = LEFT;
			tab_group.add(btn);
	...
	function addViewTab()
	{
		...
		showLastGridButton.description = 'Show/Hide Last Section';
		...
		showNextGridButton.description = 'Show/Hide Next Section';
		...
		noteTypeLabelsButton.description = 'Show/Hide Note Labels';
		...
		vortexEditorButton.description = 'ON/OFF Vortex Editor';
		...
		btn.description = 'Waveform Settings';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		btn.description = 'Go to Time/Section';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		btn.description = 'Chart Editor Theme';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Reset UI Boxes', function()
		{
			...
			characterListBox.setPosition(characterListBoxPosition.x, characterListBoxPosition.y);
			UIEvent(PsychUIBox.DROP_EVENT, btn); // to force a save
		}, btnWid);
		btn.description = 'Reset UI Boxes Position';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
	}

	function addCharListTab()
	{
		var tab = characterListBox.getTab('Character List');
		var tab_group = tab.menu;
		var btnWid = 80;
		var btnX = characterListBox.width - btnWid - 10;
		var btnY = 0;

		charListRadioGroup = new PsychUIRadioGroup(10, 10, characterList, 25, 7, false, 100);
		charListRadioGroup.onClick = function()
		{
			new FlxTimer().start(0.01, function(_)
			{
				...
				var willSelectedNotes:Array<MetaNote> = [];
				for (note in notes)
					if (note.songData[1] >= startLeftIdx && note.songData[1] <= endLeftIdx)
						willSelectedNotes.push(note);
				selectedNotes = willSelectedNotes;

				loadSection();
				trace('Selected character: ${charListRadioGroup.checkedRadio.label}');
			});
		}
		tab_group.add(charListRadioGroup);

		btnY += 10;
		var charEditButton:PsychUIButton = new PsychUIButton(btnX, btnY, 'Edit', function()
		{
			var selected:Int = charListRadioGroup.checked;
			if (selected < 0)
				return;

			openSubState(new BasePrompt(270, 284, 'Edit Character', function(state:BasePrompt)
			{
				var characterData = PlayState.SONG.characters[selected];
				var strumNotes:FlxTypedGroup<StrumNote> = new FlxTypedGroup<StrumNote>();

				function reloadStrumLineNotes()
				{
					...
					strumLineX += characterData.strumPosition[0];
					strumLineY += characterData.strumPosition[1];

					strumNotes.cameras = state.cameras;

					for (i in 0...4)
					{
						var babyArrow:StrumNote = new StrumNote(strumLineX, strumLineY, i, characterData.characterType == 'player' ? 1 : 0);
						babyArrow.downScroll = ClientPrefs.data.downScroll;
						babyArrow.alpha = characterData.strumVisible ? 0.75 : 0.25;

						if (characterData.characterType != 'player')
						{
				...
				state.add(strumNotes);
				reloadStrumLineNotes();

				var btn:PsychUIButton = new PsychUIButton(state.bg.x + state.bg.width - 40, state.bg.y, 'X', state.close, 40);
				btn.cameras = state.cameras;
				state.add(btn);

				var characterDropDown:PsychUIDropDownMenu = new PsychUIDropDownMenu(10, 100, [''], function(id:Int, character:String)
				{
				...
				characterDropDown.selectedLabel = characterData.name;
				characterDropDown.x = state.bg.x + 10;
				characterDropDown.y = state.bg.y + 100;
				characterDropDown.cameras = state.cameras;

				var characterTypeDropDown:PsychUIDropDownMenu = new PsychUIDropDownMenu(10, 100, ['OPPONENT', 'PLAYER', 'GIRLFRIEND'],
					function(id:Int, type:String)
					{
						characterData.characterType = type.toLowerCase();
						updateJsonData();
						updateHeads(true);
						loadMusic();
						reloadCharacterList();
				...
				characterTypeDropDown.selectedLabel = Std.string(characterData.characterType).toUpperCase();
				characterTypeDropDown.x = state.bg.x + state.bg.width - characterTypeDropDown.width - 10;
				characterTypeDropDown.y = state.bg.y + 100;
				characterTypeDropDown.cameras = state.cameras;

				var visibleCheckBox:PsychUICheckBox;
				visibleCheckBox = new PsychUICheckBox(0, 0, 'Visible', 100, function()
				{
					characterData.visible = visibleCheckBox.checked;
				});
				visibleCheckBox.checked = characterData.visible;
				visibleCheckBox.x = state.bg.x + 10;
				visibleCheckBox.y = characterTypeDropDown.y + characterTypeDropDown.height + 12;
				visibleCheckBox.cameras = state.cameras;

				var strumVisibleCheckBox:PsychUICheckBox;
				strumVisibleCheckBox = new PsychUICheckBox(0, 0, 'Strum Visible', 100, function()
				{
					characterData.strumVisible = strumVisibleCheckBox.checked;
					reloadStrumLineNotes();
				});
				strumVisibleCheckBox.checked = characterData.strumVisible;
				strumVisibleCheckBox.x = state.bg.x + state.bg.width - strumVisibleCheckBox.width - 10;
				strumVisibleCheckBox.y = visibleCheckBox.y;
				strumVisibleCheckBox.cameras = state.cameras;

				var noteVisibleCheckBox:PsychUICheckBox;
				noteVisibleCheckBox = new PsychUICheckBox(0, 0, 'Note Visible', 100, function()
				{
					characterData.noteVisible = noteVisibleCheckBox.checked;
					reloadStrumLineNotes();
				});
				noteVisibleCheckBox.checked = characterData.noteVisible;
				noteVisibleCheckBox.x = strumVisibleCheckBox.x;
				noteVisibleCheckBox.y = strumVisibleCheckBox.y + strumVisibleCheckBox.height + 12;
				noteVisibleCheckBox.cameras = state.cameras;

				var positionXStepper:PsychUINumericStepper = new PsychUINumericStepper(0, 0, 1, characterData.position[0], -9000, 9000, 1, 80);
				positionXStepper.onValueChange = function()
				{
					characterData.position[0] = positionXStepper.value;
				};
				positionXStepper.x = state.bg.x + 10;
				positionXStepper.y = noteVisibleCheckBox.y + noteVisibleCheckBox.height + 25;
				positionXStepper.cameras = state.cameras;

				var positionYStepper:PsychUINumericStepper = new PsychUINumericStepper(0, 0, 1, characterData.position[1], -9000, 9000, 1, 80);
				positionYStepper.onValueChange = function()
				{
					characterData.position[1] = positionYStepper.value;
				};
				positionYStepper.x = state.bg.x + 10;
				positionYStepper.y = positionXStepper.y + positionXStepper.height;
				positionYStepper.cameras = state.cameras;

				var strumPositionXStepper:PsychUINumericStepper = new PsychUINumericStepper(0, 0, 1, characterData.strumPosition[0], -9000, 9000, 1, 80);
				strumPositionXStepper.onValueChange = function()
				{
					characterData.strumPosition[0] = strumPositionXStepper.value;
					reloadStrumLineNotes();
				};
				strumPositionXStepper.x = characterTypeDropDown.x;
				strumPositionXStepper.y = positionXStepper.y;
				strumPositionXStepper.cameras = state.cameras;

				var strumPositionYStepper:PsychUINumericStepper = new PsychUINumericStepper(0, 0, 1, characterData.strumPosition[1], -9000, 9000, 1, 80);
				strumPositionYStepper.onValueChange = function()
				{
					characterData.strumPosition[1] = strumPositionYStepper.value;
					reloadStrumLineNotes();
				};
				strumPositionYStepper.x = strumPositionXStepper.x;
				strumPositionYStepper.y = positionYStepper.y;
				strumPositionYStepper.cameras = state.cameras;

				var charIndexStepper:PsychUINumericStepper = new PsychUINumericStepper(0, 0, 1, characterData.index == null ? 1 : characterData.index, 1, 99,
					0, 80);
				charIndexStepper.onValueChange = function()
				{
					characterData.index = Std.int(charIndexStepper.value);
					reloadCharacterList();
				};
				charIndexStepper.x = state.bg.x + 10;
				charIndexStepper.y = positionXStepper.y + positionXStepper.height + 40;
				charIndexStepper.cameras = state.cameras;

				state.add(visibleCheckBox);
				state.add(strumVisibleCheckBox);
				state.add(noteVisibleCheckBox);

				state.add(positionXStepper);
				state.add(positionYStepper);
				state.add(new FlxText(positionXStepper.x, positionXStepper.y - 15, 150, 'Position X/Y:'));

				state.add(strumPositionXStepper);
				state.add(strumPositionYStepper);
				state.add(new FlxText(strumPositionXStepper.x, strumPositionXStepper.y - 15, 200, 'Strum Position X/Y:'));

				state.add(charIndexStepper);
				state.add(new FlxText(charIndexStepper.x, charIndexStepper.y - 15, 200, 'Character Index:'));

				state.add(characterDropDown);
				state.add(new FlxText(characterDropDown.x, characterDropDown.y - 15, 80, 'Character:'));
				state.add(characterTypeDropDown);
				state.add(new FlxText(characterTypeDropDown.x, characterTypeDropDown.y - 15, 160, 'Character Type:'));
			}, null, function(state:BasePrompt)
			{
				//
			}));

			trace('Editing character: ${charListRadioGroup.labels[selected]}');
		}, btnWid);
		tab_group.add(charEditButton);

		btnY += 25;
		var charNewButton:PsychUIButton = new PsychUIButton(btnX, btnY, 'New', function()
		{
			...
			PlayState.SONG.characters.push(character);
			GRID_PLAYERS = PlayState.SONG.characters.length;
			updateJsonData();
			loadMusic();
			updateHeads(true);
			reloadCharacterList();
			reloadStrumNotes();
			addUndoAction(ADD_CHARACTER, {character: PlayState.SONG.characters[PlayState.SONG.characters.length - 1]});
		}, btnWid);
		charNewButton.normalStyle.bgColor = FlxColor.GREEN;
		charNewButton.normalStyle.textColor = FlxColor.WHITE;
		tab_group.add(charNewButton);

		btnY += 25;
		var charDeleteButton:PsychUIButton = new PsychUIButton(btnX, btnY, 'Delete', function()
		{
			if (PlayState.SONG.characters[charListRadioGroup.checked].characterType == 'opponent')
			{
				var opponents:Array<SwagCharacter> = PlayState.SONG.characters.filter(function(c) return c.characterType == 'opponent');
				if (opponents.length == 1)
				{
					showOutput('Error: Cannot delete the only opponent character!', true);
					return;
				}
			}
			if (PlayState.SONG.characters[charListRadioGroup.checked].characterType == 'player')
			{
				var players:Array<SwagCharacter> = PlayState.SONG.characters.filter(function(c) return c.characterType == 'player');
				if (players.length == 1)
				{
					showOutput('Error: Cannot delete the only player character!', true);
					return;
				}
			}

			var startIdx = charListRadioGroup.checked * GRID_COLUMNS_PER_PLAYER;
			var endIdx = startIdx + (GRID_COLUMNS_PER_PLAYER - 1);
			var removedNotes:Array<MetaNote> = [];
			var willMoveNotes:Array<MetaNote> = [];
			for (note in notes)
			{
				if (note.songData[1] >= startIdx && note.songData[1] <= endIdx)
					removedNotes.push(note);
				if (note.songData[1] > endIdx)
					willMoveNotes.push(note);
			}
			for (note in removedNotes)
				notes.remove(note);
			for (note in willMoveNotes)
				note.songData[1] = note.songData[1] - GRID_COLUMNS_PER_PLAYER;
			addUndoAction(DELETE_CHARACTER, {
				character: PlayState.SONG.characters[charListRadioGroup.checked],
				index: charListRadioGroup.checked,
				notes: removedNotes,
				movedNotes: willMoveNotes,
				isMoved: willMoveNotes.length > 0
			});
			PlayState.SONG.characters.remove(PlayState.SONG.characters[charListRadioGroup.checked]);
			GRID_PLAYERS = PlayState.SONG.characters.length;
		...
		charDeleteButton.normalStyle.bgColor = FlxColor.RED;
		charDeleteButton.normalStyle.textColor = FlxColor.WHITE;
		tab_group.add(charDeleteButton);
		...
		var charMoveUpButton:PsychUIButton = new PsychUIButton(btnX, btnY, 'Move Up', function()
		{
			...
			for (note in notes)
			{
				if (note == null)
					continue;
				if (note.songData[1] >= startLeftIdx && note.songData[1] <= endLeftIdx)
					willMoveLeftNotes.push(note);
				if (note.songData[1] >= startRightIdx && note.songData[1] <= endRightIdx)
					willMoveRightNotes.push(note);
			}
			for (note in willMoveLeftNotes)
			{
				note.songData[1] = note.songData[1] - GRID_COLUMNS_PER_PLAYER;
				positionNoteXByData(note);
			}
			for (note in willMoveRightNotes)
			{
				note.songData[1] = note.songData[1] + GRID_COLUMNS_PER_PLAYER;
				positionNoteXByData(note);
			}
		...
		tab_group.add(charMoveUpButton);
		...
		tab_group.add(charMoveDownButton);

		// btnY += 30;
		// var charSelectNotesButton:PsychUIButton = new PsychUIButton(btnX, btnY, 'Select Notes', function()
		// {
		// 	var idx = charListRadioGroup.checked;
		// 	var chars = PlayState.SONG.characters;
		...
		// 	loadSection();
		// }, btnWid);
		// tab_group.add(charSelectNotesButton);
	}
	...
	function saveChart(canQuickSave:Bool = true)
	{
		...
		{
			var oldChartSaved:Bool = chartSaved;
			var chartName:String = Paths.formatToSongPath(PlayState.SONG.song) + '.json';
			if (Song.chartPath != null)
	...
	public function UIEvent(id:String, sender:Dynamic)
	{
		...
		switch (id)
		{
			...
			case PsychUIBox.DROP_EVENT:
				...
				chartEditorSave.data.characterListBoxPosition = [characterListBox.x, characterListBox.y];
		}
	}
	...
	function openEditorPlayState()
	{
		...
		upperBox.isMinimized = true;
		upperBox.visible = mainBox.visible = infoBox.visible = characterListBox.visible = false;
	}
	...
	override function closeSubState()
	{
		...
		super.closeSubState();
		upperBox.isMinimized = true;
		upperBox.visible = mainBox.visible = infoBox.visible = characterListBox.visible = true;
		upperBox.bg.visible = false;
		updateAudioVolume();
	}
	...
	function undo()
	{
		...
		switch (action.action)
		{
			...
			case ADD_CHARACTER:
				try
				{
					PlayState.SONG.characters.remove(PlayState.SONG.characters[PlayState.SONG.characters.indexOf(action.data.character)]);
					GRID_PLAYERS = PlayState.SONG.characters.length;
					updateJsonData();
					loadMusic();
					updateHeads(true);
			...
			case DELETE_CHARACTER:
				try
				{
					PlayState.SONG.characters.insert(action.data.index, action.data.character);
					var swagNotes:Array<MetaNote> = action.data.notes;
					for (note in swagNotes)
					{
						if (note == null)
							continue;
						notes.push(note);
					}
					var movedNotes:Array<MetaNote> = action.data.movedNotes;
			...
			case MOVE_CHARACTER:
				try
				{
					...
					var willMoveLeftNotes:Array<MetaNote> = action.data.willMoveLeftNotes;
					for (note in willMoveLeftNotes)
					{
						if (note == null)
							continue;
					...
					var willMoveRightNotes:Array<MetaNote> = action.data.willMoveRightNotes;
					for (note in willMoveRightNotes)
					{
						if (note == null)
							continue;
