...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		['Change Camera Modulo', "Sets the camera to zoom every specified number of beats.\nValue 1: Number of beats between each zoom."],
	...
	override function create()
	{
		...
		if(chartEditorSave.data.nearestList != null) nearestList = chartEditorSave.data.nearestList;
	...
	function addFileTab()
	{
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Chart...', function()
		{
			...
			openSubState(new BasePrompt(620, 380, 'Open Chart Data',
				function(state:BasePrompt) {
					...
					for (i in 0...5)
					{
						if (realNearestList[i] != null)
						{
							...
							text.applyMarkup('{json}${jsonName}{json} - ${realNearestList[i].substr(0, 38 - (jsonName.length + 3))}...', [new FlxTextFormatMarkerPair(new FlxTextFormat(0xFF1F88D9, false, false), "{json}")]);
					...
					flixel.input.mouse.FlxMouseEvent.add(text, spr ->
					{
						...
						openSubState(new BasePrompt(420, hei, 'Choose an Nearest Chart',
							function(state:BasePrompt) {
								upperBox.isMinimized = true;
								upperBox.bg.visible = false;
