...
class TitleState extends MusicBeatState
{
	...
	public function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "startIntro", startIntro);
		Lua_helper.add_callback(lua, "skipIntro", skipIntro);

		Lua_helper.add_callback(lua, "createCoolText", createCoolText);
		Lua_helper.add_callback(lua, "addMoreText", addMoreText);
		Lua_helper.add_callback(lua, "deleteCoolText", deleteCoolText);
	}
	#end
	...
	override public function create():Void
	{
		...
		if(FlxG.save.data.flashing == null && !FlashingState.leftState)
		{
			...
			FlxTransitionableState.skipNextTransOut = true;
			MusicBeatState.switchState(MusicBeatState.getClassFromStateMap("FlashingState"));
		}
		else
		{
			startIntro();
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				var ret:Dynamic = callOnScripts('onPressedEnter', true);
				if(ret != LuaUtils.Function_Stop)
				{
					...
					FlxG.camera.flash(ClientPrefs.data.flashing ? FlxColor.WHITE : 0x4CFFFFFF, 1, null, true);
	...
	{
		var ret:Dynamic = callOnScripts('onCreateCoolText', [textArray, offset], true);
		if(ret != LuaUtils.Function_Stop)
		{
	...
	{
		var ret:Dynamic = callOnScripts('onAddMoreText', [text, offset], true);
		if(ret != LuaUtils.Function_Stop)
		{
	...
	{
		var ret:Dynamic = callOnScripts('onDeleteCoolText', null, true);
		if(ret != LuaUtils.Function_Stop)
		{
	...
	override function beatHit()
	{
		...
		if(!closedState)
		{
			...
			switch (sickBeats)
			{
				...
				case 2:
					createCoolText(['Psychness Engine by'], -120);
					addMoreText('Psych Engine by', 130);
				case 4:
					addMoreText('Nes', -140);
					addMoreText('Shadow Mario', 115);
					addMoreText('Riveren', 115);
				case 5:
					deleteCoolText();
				case 6:
	...
	{
		var ret:Dynamic = callOnScripts('onSkipIntro', null, true);
		if(ret != LuaUtils.Function_Stop)
		{
			if (!skippedIntro)
			{
